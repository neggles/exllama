...
class ExLlamaGenerator:
    class Settings:
        ...
        token_repetition_penalty_sustain = 256  # Repetition penalty window (-1 for full context)
    ...
    def make_rep_mask(self, penalty_max, sustain, decay):
        return cuda_ext.ext_rep_penalty_mask_cpu(
            self.model.config.vocab_size, self.sequence, penalty_max, sustain, decay
        )

    def batched_sample(self, logits, temperature, top_k, top_p, min_p, typical, num=1):
        if logits.shape[0] == 1:
            return self.sample(logits, temperature, top_k, top_p, min_p, typical, num)
    ...
    def sample(self, logits, temperature, top_k, top_p, min_p, typical, num=1):
        ...
        if logits.dim() == 3:
            ...
            raise ValueError("Bad logits dimension")
        ...
        if top_p > 0.0:
            ...
            while True:
                num_top_p_probs += 1
                if num_top_p_probs == top_probs.shape[-1]:
                    break
                if top_probs[num_top_p_probs].item() < min_p:
                    break
        ...
        if typical > 0.0:
            ...
            while True:
                num_typical_probs += 1
                if num_typical_probs == top_probs.shape[-1]:
                    break
        ...
        sampled_ind = torch.multinomial(
            top_probs, top_probs.shape[-1] if num == -1 else min(num, top_probs.shape[-1])
        )
    ...
    def gen_begin_reuse(self, in_tokens):
        ...
        reuse = 0
        while (
            reuse < self.sequence.shape[-1]
            and reuse < in_tokens.shape[-1]
            and self.sequence[0, reuse] == in_tokens[0, reuse]
        ):
            reuse += 1
    ...
    def gen_prune_right(self, tokens):
        self.end_beam_search()
        if tokens > self.sequence.shape[-1] - 1:
            return
    ...
    def gen_prune_to(self, min_tokens_to_keep, token_id):
        ...
        if self.gen_num_tokens() <= min_tokens_to_keep:
            return
    ...
    def gen_single_token(self, constraints=None):
        ...
        if self.sequence is not None:
            ...
            cuda_ext.ext_apply_rep_penalty_mask_cpu(
                self.sequence,
    ...
    class Beam:
        ...
        def to_sequence(self):
            ...
            if tlen > slen:
                ...
                self.generator.sequence = torch.cat(
                    (self.generator.sequence, self.sequence[:, -new_tokens:]), dim=1
                )
            ...
            for i in range(new_tokens_, len(self)):
                if self.generator.sequence[0, -i - 1] != self.sequence[0, -i - 1]:
                    new_tokens = i + 1
            ...
            if new_tokens > len(self) - 1:
                new_tokens = len(self) - 1
            if new_tokens > 0:
                self.cache.copy_states(
                    self.generator.cache,
                    len(self) - 1 - new_tokens,
                    new_tokens,
                    self.generator.cache.current_seq_len - new_tokens,
                    new_tokens,
                    0,
                    1,
                    0,
        ...
        def record_last_cache_column(self):
            self.generator.cache.copy_states(
                self.cache, self.generator.cache.current_seq_len - 1, 1, len(self) - 1, 1, 0, 1, 0, 1
            )

    def begin_beam_search(self):
        self.beams = None
        if self.settings.beams == 1 and self.settings.beam_length == 1:
            return
    ...
    def beam_search(self):
        ...
        max_beam_length = min(
            self.model.config.max_seq_len - self.settings.beam_length, self.settings.beam_length
        )
        while self.beams is None or len(self.beams[0]) < max_beam_length:
            if self.beams is None:
                ...
                while len(self.beams) < min(self.settings.beams, tokens.shape[-1]):
                    beam = ExLlamaGenerator.Beam(
                        self.settings, self, tokens[0, len(self.beams)], probs[0, len(self.beams)], c_seq_len
                    )
                ...
                for i in range(len(beams_all)):
                    ...
                    if not self.beams[beam_idx].moved:
                        self.beams[beam_idx].sequence = torch.cat(
                            (self.beams[beam_idx].sequence, new_token.unsqueeze(0).unsqueeze(0)), dim=1
                        )
                        self.beams[beam_idx].probs = torch.cat(
                            (self.beams[beam_idx].probs, new_prob.unsqueeze(0).unsqueeze(0)), dim=1
                        )
    ...
    def replace_last_token(self, token, seq=False):
        ...
        if seq:
            self.sequence[:, -1] = token

    def sequence_ends_with(self, tokens):
        if self.sequence_actual.shape[-1] < tokens.shape[-1] + 1:
            return False
        for i in range(tokens.shape[-1]):
            if self.sequence_actual[0, -i - 1] != tokens[0, -i - 1]:
                return False
        return True
