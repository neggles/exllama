...
class Node:
    ...
    def get_dict(self):
        dic = {"author": self.author, "text": self.text, "uuid": self.uuid}
        return dic
...
class Session:
    ...
    def __init__(self, filename, load):
        ...
        generator.settings.token_repetition_penalty_sustain = saved.get(
    ...
    def api_populate(self):
        ...
        names = [
            ...
            for f in files
            if os.path.isfile(os.path.join(s_dir, f)) and f.endswith(".json")
        ]
        names = sorted(names)
    ...
    def respond(self, author, stop_conditions, total_tokens, res_line="", num_res_tokens=0):
        ...
        packet = {"cmd": "begin_block", "uuid": new_block_uuid}
        ...
        for i in range(self.max_response_tokens):
            ...
            if generator.sequence_actual is not None:
                if (
                    generator.sequence_actual.shape[-1] + self.chunk_size + generator.settings.beam_length + 1
            ...
            for stop_tokens, stop_string in stop_conditions:
                if res_line.lower().endswith(stop_string.lower()):
                    generator.gen_rewind(
                        stop_tokens.shape[-1]
                        - (1 if stop_tokens[0, 0].item() == tokenizer.newline_token_id else 0)
                    )
        ...
        newNode = Node(
            res_line, author, node_id=new_block_uuid
        )  # TODO: Reuse generated tokens instead of reencoding, if it matters?
        self.history.append(newNode)
    ...
    def respond_multi(self, user_input):
        ...
        if len(user_input) > 0:
            ...
            packet = {"cmd": "begin_block", "init_text": user_input, "uuid": newNode.uuid}
            if author is not None:
                packet["author"] = author
            yield json.dumps(packet) + "\n"
        ...
        # Two participants
            ...
            yield from self.respond(
                self.participants[1], stop_conditions, total_tokens, res_line, num_res_tokens
            )
        ...
        print(
            f"Response generated in {elapsed:.2} seconds, {total_tokens[0]} tokens, {token_rate:.2f} tokens/second:"
        )
