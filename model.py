...
sys_version = Version(get_config_var("py_version"))

if sys_version < min_version:
    warnings.warn(f"This project requires Python {min_version} you have {sys_version}", UserWarning)
...
class ExLlamaConfig:
    ...
    def set_tuning_params(self):
        cuda_ext.exllama_ext.set_tuning_params(
...
class Ex4bitLinear:
    def __init__(self, config, in_features, out_features, has_bias, tensors, key):
        ...
        self.q4 = cuda_ext.ext_make_q4(self.qweight, self.qzeros, self.scales, self.g_idx, self.device_index)
...
class ExLlamaMLP:
    def __init__(self, config, tensors, key):
        ...
        self.gate_proj = Ex4bitLinear(
            config, self.config.hidden_size, self.config.intermediate_size, False, tensors, key + ".gate_proj"
        )
        self.up_proj = Ex4bitLinear(
            config, self.config.hidden_size, self.config.intermediate_size, False, tensors, key + ".up_proj"
        )
        self.down_proj = Ex4bitLinear(
            config, self.config.intermediate_size, self.config.hidden_size, False, tensors, key + ".down_proj"
        )
    ...
    def fused(self, x, buffer, post_attention_layernorm, lora):
        ...
        temp_size = 0
        if not gate_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * gate_a.shape[1])
        if not up_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * up_a.shape[1])
        if not down_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * down_a.shape[1])

        if temp_size > 0:
            lora_temp = torch.empty((1, temp_size), dtype=torch.float16, device=x.device)
        else:
            lora_temp = cuda_ext.none_tensor

        cuda_ext.exllama_ext.q4_mlp(
            x.view(-1, x.shape[-1]),
...
class ExLlamaAttention:
    ...
    def fused(self, hidden_states, cache, buffer, input_layernorm, lora):
        ...
        temp_size = 0
        if not q_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * q_a.shape[1])
        if not k_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * k_a.shape[1])
        if not v_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * v_a.shape[1])
        if not o_a.is_meta:
            temp_size = max(temp_size, bsz * q_len * o_a.shape[1])
        if temp_size > 0:
            lora_temp = torch.empty((1, temp_size), dtype=torch.float16, device=hidden_states.device)
        else:
            lora_temp = cuda_ext.none_tensor

        # Project q, k, v, apply position embeddings to k and v, update cache
        query_states = torch.empty(
            (bsz, q_len, self.config.hidden_size), dtype=torch.float16, device=hidden_states.device
        )
        ...
        # Output projection
        cuda_ext.exllama_ext.q4_attn_2(hidden_states, attn_output, self.o_proj.q4, o_a, o_b, lora_temp)
        # return hidden_states
    ...
    def forward(self, hidden_states, cache, buffer, lora):
        ...
        cuda_ext.exllama_ext.rope_(
            query_states, self.sin, self.cos, past_len, self.config.num_attention_heads, self.config.head_dim
        )
        cuda_ext.exllama_ext.rope_(
            key_states, self.sin, self.cos, past_len, self.config.num_attention_heads, self.config.head_dim
        )
        ...
        value_states = (
            self.v_proj.forward(hidden_states, lora)
        ...
        if self.config.sdp_thd == 0 or q_len < self.config.sdp_thd:
            ...
            attn_weights /= math.sqrt(self.config.head_dim)
            if buffer.attn_mask is not None:
                attn_weights = attn_weights + buffer.attn_mask
            attn_weights = nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float16).to(
                query_states.dtype
            )
            attn_output = torch.matmul(attn_weights, value_states)
            attn_output = attn_output.transpose(1, 2)
        ...
        # -- Scaled dot-product attention from PyTorch 2, should be comparable to xformers (?)
            ...
            if past_len > 0 or (bsz > 1 and buffer.attn_mask is not None):
                attn_output = F.scaled_dot_product_attention(
                    query_states, key_states, value_states, attn_mask=buffer.attn_mask, is_causal=False
                )
            else:
                attn_output = F.scaled_dot_product_attention(
...
class ExLlamaBuffer:
    ...
    def to(self, device):
        ...
        new.attn_mask = (
            None if self.attn_mask is None else _move_tensor(self.attn_mask, device, "attn_mask", self.config)
        )
        return new
...
class ExLlama:
    def __init__(self, config):
        ...
        with safe_open(self.config.model_path, framework="pt", device="cpu") as f:
                ...
                for layer in range(self.config.num_hidden_layers + 2):
                    ...
                    while device_usage + this_layer_size > max_usage:
                        ...
                        if device_index >= len(self.config.auto_map):
                            raise ValueError("Model too large for device allocation scheme.")
            ...
            for key in f.keys():
                ...
                if key.endswith(".qweight"):
                    max_dq_buffer_size = max(max_dq_buffer_size, tensor.numel() * 8)
        ...
        self.embed_tokens = nn.Embedding(
            self.config.vocab_size, self.config.hidden_size, self.config.pad_token_id, device="meta"
        )
        ...
        for device in devs:
            inv_freq = 1.0 / (
                ...
                ** (torch.arange(0, self.config.head_dim, 2, device=device).float() / self.config.head_dim)
            )
            t = torch.arange(self.config.max_seq_len, device=device, dtype=torch.float32)
            if self.config.compress_pos_emb != 1.0:
                t /= self.config.compress_pos_emb
        ...
        for dev in self.config.device_map.get_layers_devs():
            ...
            temp_state = torch.zeros(
                (config.max_input_len, config.intermediate_size), dtype=torch.float16, device=dev
            )
            temp_mlp = torch.zeros(
                (config.fused_mlp_thd * 2, config.intermediate_size), dtype=torch.float16, device=dev
            )
            ...
            cuda_ext.exllama_ext.prepare_buffers(
                torch.device(dev), temp_state, temp_mlp, temp_zeros_float, temp_dq
            )
    ...
    def forward(
        ...
        while chunk_begin < q_len:
            ...
            r = self._forward(
                input_ids[:, chunk_begin:chunk_end],
                cache,
                _last_id_only,
                _preprocess_only,
                lora,
        ...
        with torch.no_grad():
            ...
            if seq_len > 1:
                attn_mask = torch.zeros(
                    batch_size, 1, seq_len, past_len + seq_len, dtype=torch.float16, device=devs[0]
                )
                ...
                if input_mask is not None:
                    ...
                    input_mask = torch.where(input_mask, 0, -65504.0).half()
