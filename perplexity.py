...
from model import ExLlamaCache


# Perplexity calculation class
class Perplexity:
    # Passing in the model, cache, and tokenizer is a hack to prevent having to reinitialize/load
    # or move all of them into global shared objects (ew)
    def __init__(self, method="default", model=None, cache=None, tokenizer=None):
        self.dataset_chunks = []  # populated by load()
        ...
        if dataset_path.suffix in [".jsonl", ".json"]:
            for line in dataset_path.read_text().splitlines():
                ...
                if len(example) > minlength:
                    ...
                    if chunk_truncate is not None:
                        chunk = chunk[:, :chunk_truncate]
...
def add_args(parser):
    parser.add_argument(
        ...
        const="default",
        metavar="METHOD",
        help="Perplexity benchmark. Optionally specify method: gptq-for-llama, llama.cpp (not yet implemented)",
    )
    parser.add_argument(
        "-ppl_ds",
        "--perplexity_dataset",
        metavar="DATAPATH",
        type=str,
        help="Load dataset for perplexity (JSONL if .jsonl, otherwise parses it as raw text)",
    )
    parser.add_argument(
        "-ppl_cn",
        "--perplexity_chunk_num",
        nargs="?",
        type=int,
        help="Number of chunks for perplexity benchmark",
        default=100,
    )
    parser.add_argument(
        "-ppl_cs",
        "--perplexity_chunk_size",
        type=int,
        help="Size of chunks for perplexity benchmark",
        default=2048,
    )
    parser.add_argument(
        "-ppl_ct",
        "--perplexity_chunk_truncate",
        type=int,
        help="Truncated size of chunks for perplexity benchmark",
        default=2048,
    )
    parser.add_argument("-ppl_co", "--perplexity_chunk_overlap", type=int, help="Chunk overlap", default=0)
    parser.add_argument(
        "-ppl_cm", "--perplexity_chunk_min", type=int, help="Minimum chunk length", default=50
    )
    parser.add_argument(
        "-ppl_key",
        "--perplexity_json_key",
        type=str,
        help="Key to extract from JSON dataset, default: 'text'",
        default="text",
    )
    parser.add_argument(
        "-ppl_t",
        "--perplexity_token",
        action="store_true",
        help="Run perplexity test on individual tokens, for debug purposes (slow)",
    )
...
def post_parse(args):
    ...
    if args.perplexity_dataset is None:
        args.perplexity_dataset = "datasets/wikitext2_val_sample.jsonl"

    print(" -- Perplexity:")
    print(f" -- - Dataset: {args.perplexity_dataset}")
    print(f" -- - Chunks: {args.perplexity_chunk_num}")
    print(
        f" -- - Chunk size: {args.perplexity_chunk_size}"
        + (f" -> {args.perplexity_chunk_truncate}" if args.perplexity_chunk_truncate is not None else "")
    )
